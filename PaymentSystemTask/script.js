...
var departmentArr = unique(arr, 'DEPARTMENT');
console.log(departmentArr);

var amountDepartmentArr = [];
for (var i = 0; i < departmentArr.length; i++) {
    amountDepartmentArr[i] = summDepartmentAmount(arr, departmentArr[i]);
}
console.log("Array with Amount for Departments:  " + amountDepartmentArr);

var monthArr = unique(arr, 'MONTH');
console.log('uniqMont' + monthArr);

var amountMonthArr = getObSummMonthAverage(arr, monthArr);
console.log("Amount for Month : " + amountDepartmentArr);
...
createTable(totalSumAmount);

//---------- FUNCTIONS ----------------------

// get summ of AMOUNT by MONTH:
function getObSummMonthAverage(arr, monthArr) {

    var resMonthAverage = {};
    for (var i = 0; i < monthArr.length; i++) {
        resMonthAverage[monthArr[i]] = summMonthAverage(arr, monthArr[i]);
    }
    return resMonthAverage;
}

function summMonthAverage(arr, month) {
    var summ = 0;
    var l = 0;
...
function summDepartmentAmount(arr, departmentName) {
    var summ = 0;
    var l = 0;
    for (var i = 0; i < arr.length; i++) {
